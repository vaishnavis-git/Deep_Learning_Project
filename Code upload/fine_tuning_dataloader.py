...
class SynthTextDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            image = Image.open('./SynthText23/SynthText/'+image_path).convert("RGB")
            w, h = image.size
            image = np.asarray(image)
            ...
            length_difference = self.word_len[1] - len(text)

            if length_difference < 0:
                print(f"Skipping batch because the expected word length {self.word_len[1]} is less than the length of the text {len(text)}.")
                # You can return None or continue with a different logic here depending on your pipeline
                return None
            else:
                seg_mask = torch.cat((torch.ones(len(text)), torch.zeros(length_difference)))
